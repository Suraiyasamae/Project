...
def save_to_csv(data, filename, label):
    ...
    with open(filename, 'a', newline='') as file:  # เปิดไฟล์ CSV ในโหมดเพิ่มข้อมูล (append)
        ...
        if not file_exists:  # ถ้าไฟล์ยังไม่มีอยู่ ให้เขียนหัวข้อคอลัมน์ก่อน
            headers = ["Time Stamp"] + list(range(0, len(data))) + ["Label"]  # เพิ่มลำดับตัวเลขในแถวแรกและเพิ่มคอลัมน์ Label
            writer.writerow(headers)  # เขียนแถวแรกเป็นลำดับตัวเลข
        
        # เขียนเวลาปัจจุบันในคอลัมน์แรก และข้อมูลอุณหภูมิในคอลัมน์ถัดไป
        writer.writerow([datetime.now().strftime("%Y-%m-%d %H:%M:%S")] + data + [label])  # เพิ่ม Label ที่เลือก
...
def main():
    ...
    start_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")  # ได้วันที่และเวลาที่เริ่มต้น
    csv_file = os.path.join(folder_name, f"{start_time}.csv")  # รวมโฟลเดอร์และชื่อไฟล์เข้าด้วยกัน

    # ให้ผู้ใช้เลือก Label
    print("เลือก Label:")
    print("1 = request")
    print("2 = non-request")
    choice = input("กรุณากดหมายเลขที่ต้องการ: ")

    # ตั้งค่าตัวแปร label ตามการเลือก
    if choice == '1':
        label = 'request'
    elif choice == '2':
        label = 'non-request'
        ...
        label = 'non-request'  # ใช้ค่าเริ่มต้นถ้าตัวเลือกไม่ถูกต้อง

    start_time = time.time()  # บันทึกเวลาที่เริ่มต้น
    elapsed_time = 0  # ตัวแปรเก็บเวลาที่ใช้ไป
    while elapsed_time < 30:  # ทำซ้ำจนกว่าจะครบ 30 วินาที
        data = get_thermal_data()  # ดึงข้อมูลอุณหภูมิจาก ESP32
        if data:  # ถ้าดึงข้อมูลสำเร็จ
            save_to_csv(data, csv_file, label)  # บันทึกข้อมูลลงในไฟล์ CSV
